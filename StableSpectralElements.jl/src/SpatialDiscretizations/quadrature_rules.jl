...
struct WorkuHickenZinggQuadrature <: AbstractQuadratureRule
    degree::Int
end
...
function quadrature(::Line, quadrature_rule::WorkuHickenZinggQuadrature)
    return quadrature(Line(), LGQuadrature(ceil(Int, (quadrature_rule.degree - 1) / 2)))
end

function quadrature(::Tri, quadrature_rule::WorkuHickenZinggQuadrature)
    return WHZ_quad_nodes_tri(quadrature_rule.degree)
end

function quadrature(::Tet, quadrature_rule::WorkuHickenZinggQuadrature)
    println("Calling WHZ quads")
    return WHZ_quad_nodes_tet(quadrature_rule.degree)
end
...
function quadrature(::Quad, quadrature_rule::NTuple{2, AbstractQuadratureRule})
...
function quadrature(::Hex, quadrature_rule::NTuple{3, AbstractQuadratureRule})
    r1d_1, w1d_1 = quadrature(Line(), quadrature_rule[1])
    r1d_2, w1d_2 = quadrature(Line(), quadrature_rule[2])
...
function quadrature(::Tet, quadrature_rule::NTuple{3, AbstractQuadratureRule})
    ...
    if ((quadrature_rule[1].a, quadrature_rule[1].b) == (0, 0) &&
        ...
        0.125 * (η -> (1 - η)).(r_grid[2][:]) .* (η -> ((1 - η))^2).(r_grid[3][:]) .*
        w2d[:]
    elseif ((quadrature_rule[1].a, quadrature_rule[1].b) == (0, 0) &&
            (quadrature_rule[2].a, quadrature_rule[2].b) == (0, 0) &&
            (quadrature_rule[3].a, quadrature_rule[3].b) == (1, 0))
        return χ(Tet(), (r_grid[1][:], r_grid[2][:], r_grid[3][:]))...,
        0.125 * (η -> (1 - η)).(r_grid[2][:]) .* (η -> (1 - η)).(r_grid[3][:]) .*
        w2d[:]
    else
        @error "Chosen Jacobi weight not supported"
    end
