...
function advec2d(opertype::String, p_vec, M_vec)
    ...
    for p in p_vec
        ...
        for M in M_vec
            ...
            elseif opertype == "NodalTensor"
                reference_approximation = ReferenceApproximation(
                    NodalTensor(p), Tri(), mapping_degree=p)
            elseif opertype == "NodalOmega"
                reference_approximation = ReferenceApproximation(
            ...
            elseif opertype == "MFNodalTPSSLGL"
                reference_approximation = ReferenceApproximation(
            ...
            dt = 0.001 # overwrite dt with constant dt, otherwise, use CFL based dt
            dt_vec[Mcnt,pcnt]=dt
    ...
    open("ConvergenceStudies/DEC_1_Results/err_vs_runtime_plots/"*string(opertype)*"_M"*join(M_vec, "")*"_p"*join(p_vec, "")*"_CFL"*join(replace.(string.(CFL), "." => "p"), "")*"omega8dt0p001"*".txt", "w") do io
...
function advec3d(opertype::String, p_vec, M_vec)
    ...
    for p in p_vec
        ...
        for M in M_vec
            ...
            elseif opertype == "MFNodalTPSSLGL"
                reference_approximation = ReferenceApproximation(
                    MatrixFreeTPSSLGL(p), Tet(), mapping_degree=p)
            elseif opertype == "NodalTPSSMinimal"
                reference_approximation = ReferenceApproximation(
            ...
            elseif opertype == "NodalTensor"
                reference_approximation = ReferenceApproximation(
                    NodalTensor(p), Tet(), mapping_degree=p)
            else
                error("opertype not supported")
            end
            ...
            # solve
            tr_start = time()
            sol = solve(ode_problem, RK4(), adaptive=false, dt=dt,
            save_everystep=false,
            callback=save_callback(results_path, (0.0,0.01)))
            tr_end = time()
            run_time = tr_end - tr_start
            runtime_vec[Mcnt,pcnt] = run_time
            println("Run for p=$p, M=$M finished in ", round(run_time, digits=1), " seconds.")
    ...
    time_end = Dates.format(Dates.now(), "yyyy-mm-dd_HH:MM:SS")
    open("ConvergenceStudies/DEC_1_Results/err_vs_runtime_plots/"*string(opertype)*"_M"*join(M_vec, "")*"_p"*join(p_vec, "")*"_CFL"*join(replace.(string.(CFL), "." => "p"), "")*"3Domega8dt0p001"*".txt", "w") do io
        println(io, "Run ended at: ", time_start)
        println(io, "Run finished at: ", time_end)
        println(io, "Operator type: ", opertype)
        println(io, "p_vec: ", p_vec)
...
function euler2d(opertype::String, p_vec, M_vec)
    ...
    for p in p_vec
        ...
        for M in M_vec
            ...
            dt = 0.0001 # overwrite dt with constant dt, otherwise, use CFL based dt
            dt_vec[Mcnt,pcnt]=dt
            ...
            tr_start = time()
            sol = solve(ode, RK4(), adaptive=false, dt=dt, save_everystep=false,
    ...
    time_end = Dates.format(Dates.now(), "yyyy-mm-dd_HH:MM:SS")
    open("ConvergenceStudies/DEC_1_Results/err_vs_runtime_plots/euler2d_dt0p0001"*string(opertype)*"_M"*join(M_vec, "")*"_p"*join(p_vec, "")*"_C_t"*join(replace.(string.(C_t), "." => "p"), "")*"_rhomin"*join(replace.(string.(rho_central), "." => "p"), "")*".txt", "w") do io
        println(io, "Run ended at: ", time_start)
        println(io, "Run finished at: ", time_end)
        println(io, "Operator type: ", opertype)
        println(io, "p_vec: ", p_vec)
...
# Inputs: operator type (string), a vector of p values to test, a vector of M values to test

# advec2d("NodalTensor",[1,2,3,4,5],[4,8,16]) 
# advec2d("ModalOmega",[1,2,3,4,5],[4,8,16])

# advec3d("ModalTensor",[2,3,4,5,6],[2,4,6,8])
# advec3d("NodalTPSSLGL",[2,3,4,5,6,7],[1,2,3])
# advec3d("ModalOmega",[2,3,4,5,6],[2,4,6,8])
# advec3d("NodalDiagE",[2,3,4,5,6],[2,4,6,8])

# euler2d("NodalTPSSLGL",[6,7,8],[1,2,4,6])
# euler2d("ModalTensor",[6,7,8],[1,2,4,8,12])
# euler2d("NodalOmega",[6,7,8],[1,2,4,8,12])
# euler2d("NodalDiagE",[6,7,8],[1,2,4,8,12])