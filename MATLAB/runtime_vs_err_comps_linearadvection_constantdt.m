[NDEerr, NDEdof, NDEdt, NDEsetup, NDErun, NDEM_vec, NDEp_vec] = parse_output_file("NodalDiagE_M2481216_p12345678_CFL0p1omega8dt0p001.txt");
[TPSSerr, TPSSdof, TPSSdt, TPSSsetup, TPSSrun, TPSSM_vec, TPSSp_vec] = parse_output_file("NodalTPSSLGL_M2481216_p12345678_CFL0p1omega8dt0p001.txt");
[NOerr, NOdof, NOdt, NOsetup, NOrun, NOM_vec, NOp_vec] = parse_output_file("NodalOmega_M2481216_p12345678_CFL0p1omega8dt0p001.txt");
[MTerr, MTdof, MTdt, MTsetup, MTrun, MTM_vec, MTp_vec] = parse_output_file("ModalTensor_M2481216_p12345678_CFL0p1omega8dt0p001.txt");
...
subplot(2,4,7)
loglog(TPSSerr(1:end-1,7),TPSSrun(1:end-1,7),'-o','LineWidth',2,'MarkerSize',8) % remove last point for TPSS as the solution is unstable (dt too large)
hold on
...
subplot(2,4,8)
loglog(TPSSerr(1:end-1,8),TPSSrun(1:end-1,8),'-o','LineWidth',2,'MarkerSize',8) % remove last point for TPSS as the solution is unstable (dt too large)
hold on
