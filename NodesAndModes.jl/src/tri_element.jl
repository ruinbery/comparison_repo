...
"""
    WHZ_quad_nodes_tri(N)

Returns SBP-Omega based on quadrature nodes from Worku, Hicken, and Zingg (WHZ): https://arxiv.org/abs/2409.02027

exactly integrates degree N polynomials on the triangle, up to degree 84, giving SBP-Omega operators up to degree 42
"""
function WHZ_quad_nodes_tri(N)

    if N > 84
        error("WHZ nodes on triangles only available up to polynomial degree 84")
    end

    rsw::Matrix{Float64} = readdlm(string(@__DIR__, "/QuadratureData/tri_q", N, ".txt"), ',', Float64, '\n')
    r = rsw[:, 1]
    s = rsw[:, 2]
    w = rsw[:, 3]
