...
exactly integrates degree N polynomials on the tetrahedron, up to degree 40, allowing SBP-Omega operators up to degree 20
"""
function WHZ_quad_nodes_tet(N)

    if N > 40
        error("WHZ nodes on tetrahedra only available up to polynomial degree 40")
    end

    rstw::Matrix{Float64} = readdlm(string(@__DIR__, "/QuadratureData/tet_q", N, ".txt"), ',', Float64, '\n')
    r = rstw[:, 1]
    s = rstw[:, 2]
    t = rstw[:, 3]
    w = rstw[:, 4]
